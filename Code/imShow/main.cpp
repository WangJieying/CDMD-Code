...
int display1;
using namespace std;
int firstLayerMum; int firstLayerRGB = 0;
...
void draw_image(DataManager* a_dm) {
    ...
    int max_level = *std::max_element(gray_levels.begin(), gray_levels.begin() + gray_levels.size());
     ...
     firstLayerRGB ++;
     if (firstLayerRGB == 1) firstLayerMum = a_dm->getFirstLayer();
     else if (firstLayerRGB == 2) firstLayerMum = a_dm->getFirstLayer();
          else firstLayerMum = a_dm->getFirstLayer();
        ...
        glUniform1f(sh_render->uniform("layer"), firstLayerMum / 255.0);
        glBegin(GL_QUADS);
...
int main(int argc, char *argv[]) {
    ...
    if (argc>2) //need to save lots of output images.
    {
        SaveMore = 1;
        i = argv[2];
        if (*argv[3]==char('1'))  interpolate = true; 
