...
FIELD<float>* perform_skeletonization(int level, FIELD<float>* upper_level_set) {
   
    auto skel_curr = computeSkeleton(level, upper_level_set);
...
/**change to the right version.//wang.
* Calculate the skeleton for each layer.
*/

vector<std::pair<int, skel_tree_t*>>* Image::computeSkeletons(int *firstLayer) {
    ...
    skelPrev = computeSkeleton(0, imDupePrev);
    
    //skelPrev->writePGM("skelPrev.pgm");
    ...
    for (int i = 1; i < 256; ++i) {
        ...
        if (importance[i] > 0) {
            ...
            skelCurr = perform_skeletonization(i, imDupeCurr);
            ...
            //the following code mainly process skelPrev.
